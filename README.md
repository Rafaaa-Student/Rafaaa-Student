# Halo dunia! Saya Raffasya Azka Alfarabi 👋

Saya seorang pelajar kelas 7 SMP di **Al Kautsar Bandar Lampung** yang memiliki *passion* besar di bidang rekayasa perangkat lunak, AI, dan otomatisasi. Saya suka menantang diri untuk memecahkan masalah kompleks lewat baris kode.

### 🤖 Tentang Project Utama Saya: Zenn VII (Eco-Bot Ecosystem)
Saya menggabungkan teknologi **AI, Discord Bot, dan Website Dashboard** untuk membangun sebuah ekosistem interaktif ramah lingkungan yang futuristik.

- 💰 **Gamifikasi & RPG Eco-Economy**: Fitur Dual-Currency (XP & Gold), sistem `$Shop`, `$Gacha`, dan Badge Koleksi untuk meningkatkan interaksi pengguna.
- 🌱 **Green Action System**: Sistem pelacakan aksi hijau harian, *storytelling AI*, dan `$Leaderboard` global untuk mendorong gaya hidup ramah lingkungan.
- 🤖 **Generative AI integration**: Ditenagai oleh **Google Gemini 2.0 Flash Lite** untuk asisten pintar `$Zenn` dan visualisasi jaringan pikiran *Stardust Connect* menggunakan `Vis.js`.
- 🕸️ **Scraping & Data Mining**: Fitur pencarian buku otomatis, ekstraksi data Wikipedia Indonesia, dan manajemen database buku (`$FindBooks`, `$Books`).
- 🌐 **Premium Web Dashboard**: Dilengkapi fitur login Discord OAuth2, Real-time update dengan SocketIO, tabel periodik kimia interaktif, efek visual *Magical Stardust Trail*, serta Admin Panel khusus.

---

### 🏆 Competitive Programming (CP) Grind
Koleksi 40+ solusi *competitive programming* dari Codeforces & LeetCode — dari problem *easy* yang bikin senyum sampai yang butuh otak-atik *sliding window* & *greedy*. *Write in Python with love* ☕


| Total Problems | Difficulty Range | Languages |
|:-------------:|:----------------:|:---------:|
| 40+ | 800 - 1600 Rating | Python 🐍 |

---

### 🛠️ Tech Stack & Tools
<p align="left">
  <img src="https://shields.io" />
  <img src="https://shields.io" />
  <img src="https://shields.io" />
  <img src="https://shields.io" />
  <img src="https://shields.io" />
  <img src="https://shields.io" />
</p>

### 📊 Statistik Aktivitas
<p align="left">
  <img src="https://vercel.app" alt="Statistik Fasya" width="400"/>
  <img src="https://vercel.app" alt="Bahasa Terbanyak" width="300"/>
</p>
